...
//    Port.C.SetBits(BIT_1);    // Turn on the LD5 on MAX32
//  }
  err = Spi.Open(SPI4, oMasterFlags, 15e5);   // Open the SPI4 as a master at a bitrate of 4 MHz
  if (err < 0)                // Check for errors
  {
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  Port.C.ReadBits(BIT_14);    // Read pin to initialize
  
  // Set for no internal pull up resistors on
  mCNOpen((CN_ON|CN_FRZ_OFF),CN0_ENABLE, 0);
